...
class MVRSiteVideoDisplayRooms extends Shortcode {
	...
	public function site_videoroom_display_rooms() {
		...
		foreach ( $available_rooms as $room_id ) {
			...
			?>
			<tr class="inactive" data-room-id="<?php echo esc_attr( $room_id ); ?>">
				<td>
					<?php
					$title = $display_name;
					if ( $title ) {
				...
				<td class="mvr-table-editpage">
					...
					if ( $post_id_return ) {
						...
						<form method="post" action="">
							<input name="myvideoroom_regenerate_room_name" type="hidden"
								value="<?php echo esc_attr( $room_name ); ?>" />
							<input name="myvideoroom_regenerate_page_id" type="hidden"
								value="<?php echo esc_attr( $post_id ); ?>" />

							<?php wp_nonce_field( 'myvideoroom_regenerate_page', 'nonce' ); ?>
							<i class="dashicons dashicons-image-rotate"></i>
							<input type="submit" name="submit" id="submit" class="dashicons-image-rotate"
								value="Repair" />
						</form>
						<?php
					}
