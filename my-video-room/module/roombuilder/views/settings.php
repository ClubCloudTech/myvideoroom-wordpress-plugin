	...
	<form class="myvideoroom-room-builder-settings" method="post" action="">
		<fieldset>
			...
			foreach ( $room_permissions as $option ) {
				...
				?>
				<input type="radio"
					name="<?php echo esc_attr( $html_lib->get_field_name( $slug ) ); ?>"
			...
			<fieldset>
				...
				<div class="reception-settings">
					...
					<div class="custom-video-settings">
						<label for="<?php echo esc_attr( $html_lib->get_id( 'reception_custom_video_preference' ) ); ?>">
		...
		<?php
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo Factory::get_instance( Post::class )->create_form_submit(
