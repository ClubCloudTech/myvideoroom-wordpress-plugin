...
?>
	<div class="wrap">
		...
		<p><b>Options</b>..</p>
		<ul>
			<li><i>(required)</i> Set the event details event_id sets the event info if set to 'username' the loggined in username will be used as the meeting name</li>
			<li><i>(Optional)</i> Set the event details yt_url sets the embedded keynote url i.e. a Youtube live event</li>
		</ul>
		<p><b>Example</b>..</p>
			...
			<table class="form-table" role="presentation">
				<tbody>
					...
					<tr>
						...
						<td>
							<fieldset>
								<legend class="screen-reader-text"><span>Clubcloud Moderator Token Key (Provided by Clubcloud)</span></legend>
					...
					<tr>
						...
						<td>
							<fieldset>
								<legend class="screen-reader-text"><span>Clubcloud Guest Token Key (Provided by Clubcloud)</span></legend>
					...
					<tr>
						...
						<td>
							<fieldset>
								<legend class="screen-reader-text"><span>Use User Details for registered users</span></legend>
...
{
	wp_register_script('join-script', plugins_url('js/clubcloud-video.js', __FILE__), array('jquery'), null, true);
	wp_register_script('cc-ui-script', plugins_url('js/jquery-confirm.js', __FILE__), array('jquery'), null, true);
	wp_register_script('cc_lightbox', plugins_url('js/videoLightning.js', __FILE__), array('jquery'), null, true);
	wp_register_script('cc_reception', plugins_url('js/clubcloud-reception.js', __FILE__), array('jquery'), null, true);
...
function clubcloud_shortcode($atts)
{
	...
	if (is_user_logged_in() == true && $atts['auth'] == true) {             // 1 Auth = true is set in the shortcode and the user is logged in
	...
	} elseif (is_user_logged_in() == true && $atts['auth'] == false) {         // 2 Auth = false(this is on a guest page) and the user is logged in 
		$user_id = get_current_user_id();
		$user_info =  get_userdata($user_id);
		$encdata = array(
			'username' => $user_info->user_login,
		...
		$encdata = array(                                               // 3 the user s a guest user
			'username' => 'GuestUser',
			'usertype' => 'Guest',
		);
	...
	$cc_stylepath = $cc_uploadsdir['baseurl'] . '/clubcloud/' . $atts['cc_plan_id'] . '.css';       // Sets up the css for the room maps each map has its own corresponding css this allows the map designer to descide on the look and feel
	$cc_floorplan = $cc_uploadsdir['baseurl'] . '/clubcloud/';                                   // sets the url for the map files
	...
	wp_localize_script('join-script', 'wpApiSettings', array(                                  // this sets wpApiSettings.variable in the browser client for later use
	...
	wp_enqueue_style('craigs-styles', $cc_stylepath, false, 'all');                              // sets and enwueues the styles for the rings etc -- Must be changed(next)
...
function set_connectdata()
{
	...
	if (is_object($cc_userlogin) == true) {                                                         //validates the decrypted login information if returned false the toke is stale or failed decryption
		...
		if ($cc_usertype == 'Owner') {                                                                    // processing starts for the meeting owners
			...
			$connect_data[] = array(                                                                    // sets up the connection data for the browser to connect to the conf 
				'displayname' => $cc_username,
				'jwt' => cc_get_jwt($cc_room_id, $cc_username),
				'videodomain' => $cc_video_domain['host'],
				'mailaddy' => $user->user_email,
				'myavatar' => bp_core_fetch_avatar(array('item_id' => $user->ID, 'html' => false, 'type' => 'full'))
			);
			$cc_flag = 'Owner';
		} else {
...
function cc_get_jwt($cc_roomname, $cc_user)
{
	...
	$payload = json_encode(['user_id' => $cc_user, "iss" => $clubcloud_url['host'], "iat" => idate('U'), "exp" => idate('U') + 3000, "aud" => $clubcloud_url['host'], "sub" => $clubcloud_url['host'], "room" => $cc_roomname]);
	$base64UrlHeader = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($header));  // above is the setup to generate the jwt this setss the heder information
		...
		if (isset($cc_received_data)) {
			...
			foreach ($cc_received_data as $table_i) {
				...
				foreach ($table_data_i as $participant) {
					...
					$allparticipants[] = array('RoomName' => $table_i, 'JiD' => $basicjid, 'DisplayName' => $participant->display_name, 'Role' => $participant->role, 'AvatarUrl' => bp_core_fetch_avatar(array('item_id' => $user->id, 'html' => false)));
				}
				$myrooms[] = array('currentuser' => $cc_connection_id, 'RoomName' => $table_i, 'Participants' => $allparticipants);
			}
			$response = $myrooms;
		} else {
			$response = 'Failed';
...
function cc_route($cc_current_call_id)
{
	...
	$cc_selected_user_q = $wpdb->get_row("SELECT * FROM {$cc_table_name} WHERE username_jid = '$cc_current_call_id'");
	if ($cc_selected_user_q != null) {                                    //make sure we are actually looking for a valid user
		if ($cc_selected_user_q->moveto == null) {                              // check if the user is to me moved and then execute otherwise just update the timestamp
			...
			$wpdb->update($cc_table_name, $cc_dt_update, array('id' => $cc_selected_user_q->id));       //if the user is to be moved then queue to the database
		} else {
			...
			$moveto = array(
				'move' => $moveto,                                                      // send the data back to the heartbat function
		...
		$cc_insert_call_data = array(
			'username_jid' => $cc_current_call_id,
			'lastactive' => time(),
			'activemeeting' => $room_id
		);
		$wpdb->insert($cc_table_name, $cc_insert_call_data, array('%s', '%d', '%s'));
	};
...
function cc_set_route()
{
	...
	$wpdb->update($cc_table_name, $move_update, array('username_jid' => $cc_move_user));
	echo 'Success';
	wp_die();;
}
...
function cc_set_route_in()
{
	...
	$wpdb->update($cc_table_name, $move_update, array('id' => $cc_move_user));
...
function bl_clubcloud_dbmaint_exec()
{
	...
	$timedel = time() - 300;
	$lastactive = time() - 300;
	$query = "SELECT * FROM {$cc_table_name} WHERE lastactive < '$lastactive'";
	...
	foreach ($result as $li) {
		...
		$wpdb->delete($cc_table_name, $cc_del_line, array('%d'));
	}
	$wpdb->delete($cc_table_name, $cc_del_line, array('%d'));
	$cc_table_name = $wpdb->prefix . 'cc_live_queue';
	$lastactive = time() - 30;
...
function cc_encidenc($userjasondata, $cc_type, $lifetime)
{        // the lifetime used in decryption (seconds) checks the issue time from then to now and returns true or false for valid / invalid
	$encryption_key = get_option('clubcloud_web_token_key');
	if ($cc_type == 'enc') {
		$jsoncode = json_encode(array('timestamp' => time(), 'ecrypteddata' => $userjasondata));
...
function cc_getcart()
{
	if (isset($_POST['data']) && !empty($_POST['data'])) {
		...
		foreach ($passdata as $selectedprod) {
			$proddata[] = array('product_id' => $selectedprod['product_id'], 'variation_id' => $selectedprod['variation_id'], 'quantity' => $selectedprod['quantity']);
		}
...
function cc_call_queue()
{
	...
	global $wpdb;
	$cc_table_name = $wpdb->prefix . 'cc_live_queue';
	$qdata = $wpdb->get_row("SELECT * FROM {$cc_table_name} WHERE id = '$dbid'");
	if ($qdata != null) {
		if ($qdata->moved === null) {                              // check if the user is to me moved and then execute otherwise just update the timestamp
			$cc_dt_update = array(
				'lastactive' => time()
			);
			$wpdb->update($cc_table_name, $cc_dt_update, array('id' => $qdata->id));       //if the user is to be moved then queue to the database
			$query = "SELECT * FROM {$cc_table_name} WHERE callkey = '$cc_callkey' ORDER BY id ASC";
			$result = $wpdb->get_results($query, ARRAY_A);
			...
			$moveto = array(
				'action' => 'move',
				'move' => $moveto,                                                  // send the data back to the heartbeat function
				'temptoken' => $get_cc_authkey
			);
			$deldata = array(
				'id' => $dbid
			);
			$wpdb->delete($cc_table_name, $deldata, array('%d'));
			wp_send_json($moveto);
		}
	} else {
		$lastactive = time();
		...
		$wpdb->insert($cc_table_name, $cc_insert_call_data, array('%s', '%d', '%s'));
		$returndata = $wpdb->get_row("SELECT * FROM {$cc_table_name} WHERE lastactive = '$lastactive'");
		$cc_return_update = array(
			'action' => 'insert',
			'dbid' => $returndata->id,
		);
...
function cc_get_call_queue()
{
	...
	global $wpdb;
	$cc_table_name = $wpdb->prefix . 'cc_live_queue';
	$query = ("SELECT id,guestname FROM {$cc_table_name} WHERE callkey = '$cc_call_q'");
	$qdata = $wpdb->get_results($query, ARRAY_A);
	$returndata = array();
	foreach ($qdata as $ccitem) {
		$returndata[] = array('guestqid' => $ccitem['id'], 'guestname' => $ccitem['guestname']);
	}
	wp_send_json($returndata);
	wp_die();
}
