...
function clubcloud_shortcode( $atts ) {
	...
	wp_localize_script(
		'join-script',
		'wpApiSettings',
		array(                                  // this sets wpApiSettings.variable in the browser client for later use
...
function cc_get_jwt( $cc_roomname, $cc_user ) {
	...
	$base64UrlHeader    = str_replace( array( '+', '/', '=' ), array( '-', '_', '' ), base64_encode( $header ) );  // above is the setup to generate the jwt this setss the heder information
	$base64UrlPayload   = str_replace( array( '+', '/', '=' ), array( '-', '_', '' ), base64_encode( $payload ) );       // sets the payload for the video platform
